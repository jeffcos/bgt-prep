...
// ─── RECYCLE BIN VIEW ───
export function RecycleBinView({events, ops}){
  const deletedEvents = events.filter(e => e.deleted);
  return(
    <div>
      <div className="mgr-page-title">Recycle Bin</div>
      <div className="mgr-page-sub">{deletedEvents.length} deleted event{deletedEvents.length!==1?"s":""} · Owner access only</div>
...
// Keep default MgrApp shell component just in case it is imported anywhere else, but fully deprecate its use in App.jsx
    ...
    <>
      ...
      <div className="mgr-layout">
        <div className="mgr-content" style={{maxWidth:"none",width:"100%"}}>
          ...
          {tab==="guide"&&<GuideView/>}
          {tab==="recycle"&&isOwner&&<RecycleBinView events={events} ops={ops}/>}
